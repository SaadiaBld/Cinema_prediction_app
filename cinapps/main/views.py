...
def home_page(request):
    ...
    if conn:
        try:
            ...
            # obtiens les prédictions pour les films
            films = get_predictions(films)

            # vérifie si prédition_entrees est un entier (erreur lors du tri possible)
            for film in films:
                prediction_entrees = film.get('prediction_entrees', 0)
                if not isinstance(prediction_entrees, int):
                    try:
                        film['prediction_entrees'] = int(prediction_entrees)
                    except ValueError:
                        film['prediction_entrees'] = 0  # valeur par défaut si erreur
            ...
            # Obtention de la date de début de la semaine
            now = datetime.now()
            start_of_week = now - timedelta(days=7)

            # garde uniquement les films de la semaine
            films_of_the_week = [film for film in films_sorted if (film['date_sortie'] if isinstance(film['date_sortie'], date) else datetime.strptime(film['date_sortie'], '%Y-%m-%d').date()) >= start_of_week.date()]
            
            # top 10
            top_ten_films = films_of_the_week[:10]
            
            # top 2
            top_two_films = films_of_the_week[:2]
